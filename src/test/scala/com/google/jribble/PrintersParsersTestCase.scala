...
//  property("cast") = checkPrinterParser(printers.CastPrinter, parsers.cast)
  ...
  //TODO(grek): See comment above instanceOf and cast
//  property("fieldRef") = checkPrinterParser(printers.FieldRefPrinter, parsers.fieldRef)
...
//  property("arrayRef") = checkPrinterParser(printers.ArrayRefPrinter, parsers.Expressions.arrayRef)
  ...
  property("while") = checkPrinterParser(printers.WhilePrinter, parsers.whileStatement)

  property("continue") = checkPrinterParser(printers.ContinuePrinter, parsers.continueStatement)

  property("break") = checkPrinterParser(printers.BreakPrinter, parsers.breakStatement)

  property("switch") = checkPrinterParser(printers.SwitchPrinter, parsers.switchStatement)

  property("return") = checkPrinterParser(printers.ReturnPrinter, parsers.returnStatement)

  property("throw") = checkPrinterParser(printers.ThrowPrinter, parsers.throwStatement)
